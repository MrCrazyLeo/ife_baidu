...
（此仓库曾经是百度前端技术学院的作业代码仓库，因记录事项太多➕排序不当，已经迁移到daily-life仓库）
...
- 304状态码
  ...
  服务器执行了Get请求，但是文件未改变，因此返回状态码304告知客户端。主要依赖于服务器比较Last Modified 和 If Modified Since字段

  4xx是请求错误，比如404是服务器上找不到该资源；403是服务器拒绝了请求；401是无权访问

  5xx是服务器错误，比如504是网关超时；500是服务器内部错误，无法完成请求；503是服务器目前无法使用（通常是暂时状态）
...
- [TCP 为什么三次握手而不是两次握手（正解版）](https://blog.csdn.net/lengxiao1993/article/details/82771768)
  ...
  - 2次握手会让服务器尝试去服务可能已经失效的服务对象，从而造成带宽资源浪费
  - 核心是实现可靠传输
...
- undefined与null的区别
  ...
  - undefined字面意思为未定义的值。undefined 一般都来自于某个表达式最原始的状态值，不是人为操作的结果。当然，你也可以手动给一个变量赋值 undefined，但这样做没有意义，因为一个变量不赋值就是 undefined 。undefined可以在以下场景出现：
    ...
    - 函数没有返回值时，默认会返回undefined

  - null字面意思为空值，表示一个被人为充值为空的对象，而非一个变量最原始的状态。在内存里表示就是，栈中变量没有指向堆中的内存对象。当一个对象被设置为null，会被GC自动回收

    ![img](https://images2017.cnblogs.com/blog/849589/201708/849589-20170810192309683-590729765.png)

  - 为什么typeof null === 'onject'：
  
    - 因为JavaScript数据类型在底层都是以二进制的形式表示的，二进制的前三位为0会被typeof判断为对象类型，而null的二进制恰好都是0，因为被误判为Object类型
    - 使用Object.prototype.toString.call(null)就能正确识别出为[object Null]
    
  - undefined与null是所有类型的子类型
...
- 栈和堆和其他
    ...
    - 操作系统中的堆和栈是指内存空间，不同的是堆为按需申请、动态分配，例如C中malloc函数和C++中的new操作（当然C++中的new不仅仅是申请内存那么简单）。内存中的空闲空间并不是连续的，而是不同程序占用了不同的一块一块内存，即使同一个程序也可能占用了不同地方的内存。操作系统中则会对这些空间进行统一的管理，在应用程序提出申请的时候，就会从堆中按照一定的算法找出一块可用内存，标记占用空间等信息之后返回其起始地址给程序。在程序结束之前，操作系统不会删除已经申请的内存，而是要靠程序主动提出释放的请求（free、delete），如果使用后忘记释放，就会造成所谓内存泄露的问题。因此堆基本上可以理解为当前可以使用的空闲内存空间，其申请和释放都要程序员自己写代码管理。堆是向高地址扩展的数据结构，是不连续的内存区域。
    - 操作系统的栈则是程序运行时自动拥有的一小块内存，大小在编译期时由编译器参数决定，用于局部变量的存放或者函数调用栈的保存。在C中如果声明一个局部变量（例如 int a），它存放的地方就在栈中，而当这个局部变量离开其作用域之后，所占用的内存则会被自动释放，因此在C中局部变量也叫自动变量。栈的另一个作用时保存函数调用栈，这时就跟数据结构的栈有关系了。在函数调用过程中，常常会多层甚至递归调用。每一个函数调用都有各自的局部变量和返回值，每一次函数调用其实是先将当前函数的状态压栈，然后在栈顶开辟新空间用于保存新的函数状态，接下来才是函数执行。当函数执行完毕之后，栈先进先出的特性使得后调用的函数先返回，这样可以保证返回值的有序传递，也保证函数现场可以按顺序恢复。操作系统的栈在内存中高地址向低地址增长，即低地址为栈顶，高地址为栈底。这就导致了栈的空间有限制，一旦局部变量申请过多（例如超大数组），或者函数调用太深（例如递归太多次），都将导致栈溢出，操作系统这时会把程序杀掉。
    - 全量区（静态区）：全局变量和静态变量的是放在一起的，初始化的全局变量和静态变量在一块区域，未初始化的全局变量和未初始化的静态变量在相邻的另一块区域，程序结束后由系统释放。
...
- 今天接到一个前端截图并支持批注的业务需求，感觉挺有意思的。逛到一个不错的[项目](https://github.com/kejiacheng/kscreenshot)，准备修改下源码。

- git clone很慢

  - 法1：（在我机器设置了host之后没用）git clone特别慢是因为`github.global.ssl.fastly.net`域名被限制了。

    因此，只要找到你当前线路最快的ip，修改一下host就能提速。

    在网站 [https://www.ipaddress.com](https://link.zhihu.com/?target=https%3A//www.ipaddress.com/) 分别找这两个域名所对应的最快IP地址
  ...
  - 法2：使用github的镜像网站进行访问，[github.com.cnpmjs.org](https://link.zhihu.com/?target=http%3A//github.com.cnpmjs.org)，我们将原本的网站中的github.com 进行替换 —— 简单粗暴!

  - 法3：（在我机器没效果，结果还要删除刚设置的proxy）
...
- 重排跟重绘 https://juejin.im/entry/6844903453966204936



# 2020.08.15

- 以搭电梯为例子形象说明防抖与节流（假设电梯在5秒内会执行关门动作）

  - 防抖：如果A在10:00进入电梯，如果没有人在5秒内进来，那么电梯将在10：05关门；如果此时B在10：04进来，那么将重新计时，电梯将在10：09关门；
  - 节流：如果A在10:00进入电梯，如果没有人在5秒内进来，那么电梯将在10：05关门；如果此时B在10：04进来，电梯同样在10：05关门；电梯关门时间在第一个人进入电梯开始计时，不管5秒内进来多少人，电梯都将在第5秒准时关门；如果没有人进来，则电梯不关门
  - 适用场景：防抖——鼠标click、拖拽事件等、键盘输入事件、请求服务端事件、window resize事件；节流——滚动条滚动事件、鼠标不断点击触发事件、mouseover事件

- github没走代理：修改pac.txt，添加
...
- 什么是“前端工程化”？
  ...
  - 工程化主要分为模块化、组件化、规范化和自动化
  - 模块化核心是提供多人协作的可能，它将一个大的项目拆分成互相依赖的小项目，然后进行拼装和加载。这里，包含JS的模块化、CSS的模块化和资源的模块化
  - 组件化 ≠ 模块化！！模块化只是文件层面上，对代码或资源进行拆分。组件化则是再设计层面上，根据UI功能模块进行拆分。
