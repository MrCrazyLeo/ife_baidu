...
- 栈和堆和其他
    ...
    - 操作系统中的堆和栈是指内存空间，不同的是堆为按需申请、动态分配，例如C中malloc函数和C++中的new操作（当然C++中的new不仅仅是申请内存那么简单）。内存中的空闲空间并不是连续的，而是不同程序占用了不同的一块一块内存，即使同一个程序也可能占用了不同地方的内存。操作系统中则会对这些空间进行统一的管理，在应用程序提出申请的时候，就会从堆中按照一定的算法找出一块可用内存，标记占用空间等信息之后返回其起始地址给程序。在程序结束之前，操作系统不会删除已经申请的内存，而是要靠程序主动提出释放的请求（free、delete），如果使用后忘记释放，就会造成所谓内存泄露的问题。因此堆基本上可以理解为当前可以使用的空闲内存空间，其申请和释放都要程序员自己写代码管理
